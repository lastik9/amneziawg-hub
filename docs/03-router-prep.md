...
> ⚠️ **Не кодифицировано (заполнить по рабочему узлу-деревне):** procd-init самого
> `wstunnel client` (слушатель `udp:51820` → WSS до `HUB_WSS_*`) и маршрутизация
> `HUB_WSS` через прокси-ноду в Clash. Эти шаги в `setup-awg.sh` пока не входят —
> ставятся отдельно. `--wstunnel` покрывает только AWG-endpoint узла.
