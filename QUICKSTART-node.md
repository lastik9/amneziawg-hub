...
Единственное, что делается на хабе. Хаб узнаёт публичный ключ узла (из Шага 2) —
без этого туннель не встанет. Подробнее про меню — в [`QUICKSTART.md`](QUICKSTART.md)
→ «Добавь пиров».

**Запусти меню на хабе:**

```sh
cd /root && bash awg-hub.sh
```

**Пройди по пунктам** (в скобках — что ввести):

| Пункт меню | Что ввести |
|---|---|
| Раздел | `2` — Управление пирами |
| Тип пира | `+ Site` (роутер со своей LAN) |
| Имя | `test53` (любое осмысленное) |
| WG-адрес узла | `10.0.0.4` — **перебей дефолт `.2`, если предложит!** |
| LAN узла | `192.168.53.0/24` (подсеть за роутером) |
| Public key | публичный ключ **роутера**. Забыл/не записал? Покажи его на **роутере**: `grep "^PRIVATE_KEY=" /root/awg.env \| cut -d"'" -f2 \| awg pubkey` — вставь вывод сюда |
| PSK | `N` (не используем) |

> 🔑 **Не перепутай ключи.** Здесь вводишь публичный ключ **роутера** (`gwOO…`,
> роутер выдал его в Шаге 2). Обратный ключ — публичный ключ **хаба** (`ZbkJ…`) —
> уже лежит в `awg.env` роутера (`PEER_PUBLIC_KEY`, Шаг 2). Каждая сторона знает
> ПУБЛИЧНЫЙ ключ другой; приватные ключи со своей стороны не уходят.

> Пир↔пир форвардинг на хабе (узлы видят друг друга и клиентов) хаб ставит сам —
> руками ничего добавлять не надо.

**Сразу проверь, что пир записался** (на хабе):

```sh
awg show awg0 | grep -A4 peer
```

Жди строку `peer: <pubkey узла>` с `allowed ips`, включающими WG-адрес узла
(`10.0.0.4/32`) и его LAN (`192.168.53.0/24`). Handshake пока не появится — узел
ещё не поднят (Шаг 4), это нормально.

> Закрыл меню и надо вернуться (добавить ещё пир, поправить)? Открыть снова:
> ```sh
> cd /root && bash awg-hub.sh
> ```
