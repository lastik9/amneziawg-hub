...
flowchart TD
    ...
    HUB --- A["site-узел A<br>10.0.0.2<br>LAN 192.168.1.0/24"]
    HUB --- B["site-узел B<br>10.0.0.3<br>LAN 192.168.52.0/24"]
    HUB --- C["site-узел C<br>10.0.0.4<br>LAN 192.168.53.0/24"]
    HUB --- D["роуминг-клиент<br>10.0.0.10<br>Mac / телефон"]
```
...
| site A | `10.0.0.2` | `192.168.1.0/24` | роутер площадки (шлюз `.1`) |
| site B | `10.0.0.3` | `192.168.52.0/24` | роутер площадки (шлюз `.1`) |
| site C | `10.0.0.4` | `192.168.53.0/24` | роутер площадки (шлюз `.1`) |
...
- Адреса и подсети у всех **разные** — это лишь пример. `scripts/awg-hub.sh` спрашивает WG-сеть/порт/endpoint при bootstrap и WG-адрес + LAN-подсеть при добавлении каждого узла; ничего не зашито жёстко.
