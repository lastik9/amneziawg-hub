...
```bash
ssh-keygen -t ed25519 -C "awg-hub" -f ~/.ssh/awg_hub
```

- `ed25519` — современный, короткий, быстрый (не используй RSA-2048).
- Задай пароль на ключ (passphrase) — это защита, если ноут украдут.

Удобная запись в `~/.ssh/config` (чтобы ходить просто `ssh awg`):

```
Host awg
    HostName <SERVER_IP>
    User root
    IdentityFile ~/.ssh/awg_hub
    IdentitiesOnly yes
```
...
```bash
ssh-copy-id -i ~/.ssh/awg_hub.pub root@<SERVER_IP>
```

Если `ssh-copy-id` нет — вручную:

```bash
cat ~/.ssh/awg_hub.pub | ssh root@<SERVER_IP> \
...
```bash
ssh awg            # или: ssh -i ~/.ssh/awg_hub root@<SERVER_IP>
```
