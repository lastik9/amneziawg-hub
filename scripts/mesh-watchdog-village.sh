#!/bin/sh
# mesh-watchdog — ДЕРЕВНЯ (awg_hub52), дорога A (AWG-in-WSS через clash+wstunnel)
# Восстановлен из спецификации handoff §WATCHDOG (v20) после инцидента затирания v23.
# Лесенка: хендшейк протух -> restart clash -> пауза -> restart wstunnel -> пауза
#          -> всё ещё мёртв -> reboot (не чаще 1 раза в час).
# GUARD: если это не awg_hub52 — выходим без единого действия (защита от запуска не на том узле).
EXPECT_IFACE="awg_hub52"
IFACE="awg_hub52"
MAX_AGE=300
